"""Work-queueing routes: mail ingest fans out to one sync run per connected
account (any provider) and comes back 202 with the whole batch, and
classification backfills enqueue a Celery task once the batch is too big to
run inline.
"""
...
def test_ingest_requires_auth():
    resp = TestClient(app).post("/api/v1/mail/ingest")
    assert resp.status_code == 401


def test_ingest_still_validates_max_results(client):
    assert client.post("/api/v1/mail/ingest?max_results=0").status_code == 422
    assert client.post("/api/v1/mail/ingest?max_results=10000").status_code == 422
...
def test_ingest_fans_out_across_providers(fanout, monkeypatch):
    # The fan-out is no longer gmail-only -- a connected outlook account must
    # get its own run too, dispatched via its own provider.
    client, db = fanout
    gmail_account = _account(provider="gmail")
    outlook_account = _account(provider="outlook")
    db.execute.return_value.scalars.return_value.all.return_value = [
        gmail_account,
        outlook_account,
    ]
    ...
    resp = client.post("/api/v1/mail/ingest")
    assert resp.status_code == 202
    assert captured == {gmail_account.id: "gmail", outlook_account.id: "outlook"}
...
def test_ingest_custom_params_map_to_refresh_mode(fanout, monkeypatch):
    ...
    resp = client.post(
        "/api/v1/mail/ingest?max_results=50&skip_existing=false&classify=false"
    )
    assert resp.status_code == 202
...
def test_ingest_new_only_maps_to_auto_mode(fanout, monkeypatch):
    ...
    resp = client.post("/api/v1/mail/ingest?new_only=true")
    assert resp.status_code == 202
...
def test_ingest_provider_account_ids_targets_only_listed_accounts(fanout, monkeypatch):
    ...
    # the route's user/not-paused predicates already scope the SQL, so a3
    # simply never comes back from the (mocked) query.
    client, db = fanout
    ...
    resp = client.post(
        f"/api/v1/mail/ingest?provider_account_ids={account_a1.id}&provider_account_ids={account_a2.id}"
    )
    assert resp.status_code == 202
...
def test_ingest_provider_account_ids_still_skips_paused_accounts(fanout, monkeypatch):
    ...
    resp = client.post(f"/api/v1/mail/ingest?provider_account_ids={paused.id}")
    assert resp.status_code == 202
    assert resp.json() == {"runs": []}
    assert calls == []
...
def test_ingest_unknown_provider_account_id_returns_empty_runs(fanout):
    ...
    client, db = fanout
    db.execute.return_value.scalars.return_value.all.return_value = []
    resp = client.post(f"/api/v1/mail/ingest?provider_account_ids={uuid4()}")
    assert resp.status_code == 202
    assert resp.json() == {"runs": []}
