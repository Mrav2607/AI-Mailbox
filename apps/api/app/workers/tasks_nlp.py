...
def classify_message(message_id: str) -> dict:
    with SessionLocal() as db:
        ...
        # classifier.py's `_classify_llm` already degrades for -- here it
        # just means skip recording rather than an AttributeError after
        # `upsert_classification` already ran.
        usage_recorded = (
            thread is not None
